  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-500 flex flex-col gap-8 pb-10">
      ...
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-black tracking-tight text-foreground">{t("step1.title")}</h1>
        <p className="text-muted-foreground text-lg leading-relaxed">{t("step1.subtitle")}</p>
      </div>
      ...
      <div className="flex flex-col gap-8 mt-4">
        ...
        <div className="flex flex-col gap-3">
          <label className="text-base font-semibold text-foreground">{t("step1.age")}</label>
          <div className="flex items-center justify-between bg-card border border-border p-2 rounded-xl shadow-sm transition-colors">
            <button onClick={() => setPhysicalData({ age: Math.max(1, physical.age - 1) })} className="w-12 h-12 flex items-center justify-center rounded-lg bg-primary/10 text-primary hover:bg-primary/20 transition-colors">
            ...
            <button onClick={() => setPhysicalData({ age: physical.age + 1 })} className="w-12 h-12 flex items-center justify-center rounded-lg bg-primary/10 text-primary hover:bg-primary/20 transition-colors">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="relative bg-card border border-border rounded-xl p-6 shadow-sm overflow-hidden transition-colors">
            <input type="range" min="100" max="250" value={physical.height} onChange={(e) => setPhysicalData({ height: parseInt(e.target.value) })} className="absolute inset-x-0 bottom-4 w-full h-2 bg-transparent appearance-none cursor-pointer accent-primary" />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="relative bg-card border border-border rounded-xl p-6 shadow-sm transition-colors">
            <input type="range" min="40" max="200" step="0.5" value={physical.weight} onChange={(e) => setPhysicalData({ weight: parseFloat(e.target.value) })} className="w-full h-2 bg-secondary rounded-lg appearance-none cursor-pointer accent-primary" />
          </div>
        </div>

        {/* Género */}
        <div className="flex flex-col gap-3">
          <label className="text-base font-semibold text-foreground">{t("step1.gender")}</label>
          <div className="flex gap-3">
            <button onClick={() => setPhysicalData({ gender: 'MALE' })} className={`flex-1 py-4 rounded-xl border-2 font-bold transition-all ${physical.gender === 'MALE' ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:border-primary/50'}`}>
              {t("gender.MALE")}
            </button>
            <button onClick={() => setPhysicalData({ gender: 'FEMALE' })} className={`flex-1 py-4 rounded-xl border-2 font-bold transition-all ${physical.gender === 'FEMALE' ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:border-primary/50'}`}>
              {t("gender.FEMALE")}
            </button>
          </div>
        </div>
        ...
        {/* Actividad Física */}
        <div className="flex flex-col gap-3">
          <label className="text-base font-semibold text-foreground">{t("step1.activityLevel")}</label>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {activityOptions.map((level) => (
              <button key={level} onClick={() => setPhysicalData({ activityLevel: level })} className={`p-3 rounded-xl border-2 text-sm font-semibold transition-all flex items-center justify-center text-center h-16 ${physical.activityLevel === level ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:border-primary/50'}`}>
